   ...
   <c:import url="/etc/nav" />  
    <!-- End Navbar -->
     <div class="container-fluid py-3">
       <div class="row">
       <c:forEach var="infocnt" items="${info_count}" varStatus="status">
          <div class="col-md-4 mt-4">
          <div class="card">
            <div class="card-header pb-0 px-3">
                <c:if test="${infocnt.infoid==1}">
              <h6 class="mb-0">인텔 ${infocnt.infoname}</h6>
              </c:if>
               <c:if test="${infocnt.infoid==2}">
              <h6 class="mb-0">AMD ${infocnt.infoname}</h6>
              </c:if>
            </div>
            <div class="card-body pt-4 p-3">
              <ul class="list-group">
          	<c:set var="infonum" value="info${infocnt.infoid}"></c:set>
			<!-- requestScope 를 이용한 동적변수 명 받음 -->
			<c:forEach var="info" items="${requestScope[infonum]}">
                <li class="list-group-item border-0 d-flex p-2 mb-2 bg-gray-100 border-radius-lg">
                  <div class="d-flex flex-column">
                      <h6 class="mb-3 text-sm">${info.name}</h6>
                    <span class="mb-2 text-xs">소켓: <span class="text-dark font-weight-bold ms-2">${info.socket} </span></span>
                    <span class="mb-2 text-xs">제조공정: <span class="text-dark ms-2 font-weight-bold"> ${info.manufacture } </span></span>
                    <span class="mb-2 text-xs">코어: : <span class="text-dark ms-2 font-weight-bold"> ${info.core } </span></span>
                    <span class="mb-2 text-xs">쓰레드: <span class="text-dark ms-2 font-weight-bold"> ${info.thread } </span></span>
                  </div>
                </li>
              </c:forEach>
              </ul>
			...
			<div class="col-md-4 mt-4">
			    <div class="card">
			        ...
			        <div class="card-body pt-4 p-3">
			            <ul class="list-group">
			            <c:forEach var="GpuList" items="${GpuList}">
			                <li class="list-group-item border-0 d-flex p-2 mb-2 bg-gray-100 border-radius-lg">
			                    <div class="d-flex flex-column">
			                        <h6 class="mb-3 text-sm">${GpuList.name}</h6>
			                        <span class="mb-2 text-xs"><span class="text-dark font-weight-bold ms-2">${GpuList.value }</span></span>
			                        <span class="mb-2 text-xs">제조공정: <span class="text-dark ms-2 font-weight-bold"> ${GpuList.manufacture } </span></span>
			                        <span class="mb-2 text-xs">Stream: : <span class="text-dark ms-2 font-weight-bold"> ${GpuList.stream }  </span></span>
			                        <span class="mb-2 text-xs">Ram: <span class="text-dark ms-2 font-weight-bold"> ${GpuList.ram }</span></span>
			                    </div>
			                </li>
			             </c:forEach>
			            </ul>
						...
						<div class="card-body pb-0">
							<div class="table-responsive">
								<table class="table align-items-center table-sm">
									...
									<c:set var="boardnum" value="board${cnt.boardid}"></c:set>
									<!-- requestScope 를 이용한 동적변수 명 받음 -->
						...
						<div class="card-body pb-0">
							<div class="table-responsive">
								<table class="table align-items-center table-sm">
									...
									<tbody>
									<c:forEach var="list" begin="0" end="9" step="1" items="${list}">
										<tr>
											<td class="text-sm ml-1 col-md-10">
											<img src="${root}/img/small-logos/hipchat.svg" class="avatar avatar-sm">
